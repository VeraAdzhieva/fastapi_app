...
from src.infrastructure.database.model import UsersDBModel, RoleDBModel
from src.domain.models.role import Role, Permission, Object, Action
...
class SqlAlchemyUsersRepository(UsersRepository):
    ...
    def get(self, username: str) -> Optional[UserAggregate]:
        ...
        """
        db_model = (
            self.session.query(UsersDBModel).filter_by(username=username).first()
        )
        if not db_model:
            return None
    ...
    def add(self, user_agg: UserAggregate) -> None:
        ...
        role_names = [role.name if hasattr(role, 'name') else role for role in root.roles]
        db_roles = self.session.query(RoleDBModel).filter(
            RoleDBModel.name.in_(role_names)
        ).all()
