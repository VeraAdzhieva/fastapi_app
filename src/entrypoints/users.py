...
from src.application.commands import LoginIn, Register
...
from src.infrastructure.auth.token_service import ACCESS_TOKEN, TokenService
...
class RegisterDTO(BaseModel):
    username: str = Field(
        ..., description="Логин пользователя", min_length=3, max_length=50
    )
...
class LoginInDTO(BaseModel):
    username: str = Field(
        ..., description="Имя пользователя", min_length=3, max_length=50
    )
    password: str = Field(..., description="Пароль", min_length=6, max_length=50)
...
async def me(user_model: CurrentUser) -> UserInfo:
    return UserInfo(
